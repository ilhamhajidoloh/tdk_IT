...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!tokenRes.ok || !tokenData.id_token) {
      return fail("ไม่สามารถยืนยันตัวตนกับ LINE ได้");
    }

    let email = "";
    const payloadSegment = tokenData.id_token.split(".")[1];
    if (payloadSegment) {
      const decoded = JSON.parse(Buffer.from(payloadSegment, "base64").toString("utf-8")) as { email?: string };
      email = decoded.email?.toLowerCase().trim() || "";
    }

    if (!email) {
      return fail("ไม่พบอีเมลจากบัญชี LINE");
    }
    ...
    console.error("LINE link callback error:", err);
    return fail("เกิดข้อผิดพลาดในการเชื่อมต่อกับ LINE");
  }
}
