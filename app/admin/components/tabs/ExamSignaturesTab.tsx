...
const COPY = {
  th: {
    ...
    examDate: "วันที่สอบ", printPdf: "พิมพ์ / บันทึก PDF", fontSize: "ขนาดตัวอักษร", noStudents: "ไม่พบรายชื่อนักเรียนในห้องนี้",
  },
  "ms-rumi": {
    ...
    examDate: "Tarikh", printPdf: "Cetak / Simpan PDF", fontSize: "Saiz Fon", noStudents: "Tiada pelajar dalam kelas ini",
  },
  "ms-jawi": {
    ...
    examDate: "تاريخ", printPdf: "چتق / سيمڤن PDF", fontSize: "ساءيز فون", noStudents: "تياد ڤلاجر دالم کلس اين",
  },
} as const;
...
export default function ExamSignaturesTab({ settingsList, token, schoolId }: {
  ...
  const printDocument = () => {
    ...
    win.document.write(`<!doctype html><html lang="${language === "th" ? "th" : "ms"}" dir="${direction}"><head><meta charset="utf-8"><title>${text.examSheet}</title>
      ...
      <style>:root{--font-scale:1}*{box-sizing:border-box}body{font-family:${language === "ms-jawi" ? "Amiri" : "Sarabun"},sans-serif;color:#111827;margin:0;background:#f1f5f9}.toolbar{position:fixed;top:16px;right:16px;z-index:2;display:flex;gap:8px;align-items:center}.font-controls{display:flex;align-items:center;gap:7px;background:#fff;padding:6px 10px;border:1px solid #cbd5e1;border-radius:8px;font-size:12px;font-weight:700}.font-controls button{padding:3px 8px;background:#f1f5f9;color:#334155;border:1px solid #cbd5e1}.font-controls input{width:90px;accent-color:#4f46e5}button{background:#4f46e5;color:#fff;border:0;border-radius:8px;padding:10px 16px;font-weight:700;cursor:pointer}.page{width:210mm;min-height:297mm;margin:12px auto;background:#fff;padding:18mm 15mm;page-break-after:always}h1{text-align:center;font-size:calc(18px * var(--font-scale));margin:0 0 4px;min-height:24px}h2{text-align:center;font-size:calc(22px * var(--font-scale));margin:0 0 12px}.details{display:flex;flex-wrap:wrap;gap:8px 24px;border-top:1px solid #94a3b8;border-bottom:1px solid #94a3b8;padding:9px 0;margin-bottom:14px;font-size:calc(14px * var(--font-scale))}.details span{min-width:30%}table{border-collapse:collapse;width:100%;font-size:calc(12px * var(--font-scale))}th,td{border:1px solid #475569;padding:calc(7px * var(--font-scale)) calc(5px * var(--font-scale));vertical-align:middle}th{background:#e2e8f0;text-align:center;font-weight:700}td:first-child{width:6%;text-align:center}td:nth-child(2){width:13%;text-align:center}.name{width:29%}.score{width:9%;height:calc(38px * var(--font-scale));text-align:center}.signature{height:calc(38px * var(--font-scale));width:25%}.empty{text-align:center;color:#64748b;padding:20px}.total{text-align:${language === "ms-jawi" ? "left" : "right"};font-weight:700;margin-top:10px;font-size:calc(14px * var(--font-scale))}.signatures{display:flex;justify-content:space-between;gap:32px;margin-top:42px;font-size:calc(14px * var(--font-scale))}.signatures div{flex:1;white-space:nowrap}@media print{body{background:#fff}.toolbar{display:none}.page{margin:0;box-shadow:none;page-break-after:always}@page{size:A4 portrait;margin:0}}</style></head><body><div class="toolbar"><div class="font-controls"><span>${text.fontSize}</span><button onclick="changeFont(-10)">A−</button><input id="font-slider" type="range" min="70" max="140" value="100" oninput="setFont(this.value)"><button onclick="changeFont(10)">A+</button><span id="font-value">100%</span></div><button onclick="window.print()">${text.printPdf}</button></div>${pages}<script>function setFont(value){document.documentElement.style.setProperty('--font-scale',value/100);document.getElementById('font-slider').value=value;document.getElementById('font-value').textContent=value+'%'}function changeFont(delta){var slider=document.getElementById('font-slider');setFont(Math.max(70,Math.min(140,Number(slider.value)+delta)))}</script></body></html>`);
    win.document.close();
  };
